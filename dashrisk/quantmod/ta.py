...
# import numpy as np
# import pandas as pd
from quantmod import tanolib as talib
from . import utils
from .valid import VALID_TA_KWARGS


def add_(self, study_name,values,timeperiod=26,
             type='line', color='secondary', **kwargs):
    """Basic routine for all studies."""
    ...
    name = '{}({})'.format(str(study_name),str(timeperiod))
    self.pri[name] = dict(type=type, color=color)
    self.ind[name] = values
...
def add_MA(self, timeperiod=20, matype=0,
    ...
    study_name = 'MA({})'.format(str(timeperiod))
    values = self.df[self.cl].rolling(timeperiod).mean()
    add_(self,study_name, values, timeperiod, type, color)
...
#     self.pri[name] = dict(type=type, color=color)
#     self.ind[name] = talib.MA(self.df,
#                               timeperiod)

#     movavg = self.df[self.cl].rolling(timeperiod).mean()
#     self.ind[name] = movavg.values
...
def add_SMA(self, timeperiod=20,
    ...
    study_name = 'SMA({})'.format(str(timeperiod))
    values = self.df[self.cl].rolling(timeperiod).mean()
    add_(self,study_name, values, timeperiod, type, color)
...
#                                timeperiod)
    self.ind[name] = talib.EMA(self.df,
                               timeperiod)
...
def add_WMA(self, timeperiod=20,
    ...
    self.pri[name] = dict(type=type, color=color)
    self.ind[name] = talib.WMA(self.df,
                               timeperiod)
...
def add_DEMA(self, timeperiod=26,
    ...
    self.pri[name] = dict(type=type, color=color)
    self.ind[name] = talib.DEMA(self.df,
                                timeperiod)
...
def add_TEMA(self, timeperiod=26,
    ...
    self.pri[name] = dict(type=type, color=color)
    self.ind[name] = talib.TEMA(self.df,
                                timeperiod)
...
def add_BBANDS(self, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0,
    ...
    (self.ind[ubb],
     self.ind[bb],
     self.ind[lbb]) = talib.BBANDS(self.df,
...
def add_AROON(self, timeperiod=14,
    ...
    self.ind[uaroon], self.ind[daroon] = talib.AROON(self.df,
                                                     timeperiod)
...
def add_CCI(self, timeperiod=14,
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.CCI(self.df,
                               timeperiod)
...
def add_ADX(self, timeperiod=14,
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.ADX(self.df,
                               timeperiod)
...
def add_MACD(self, fastperiod=12, slowperiod=26, signalperiod=9,
    ...
    (self.ind[macd],
     self.ind[smacd],
     self.ind[hmacd]) = talib.MACD(self.df,
...
def add_MFI(self, timeperiod=14,
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.MFI(self.df,
                               timeperiod)
...
def add_MOM(self, timeperiod=10,
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.MOM(self.df,
                               timeperiod)
...
def add_ROC(self, timeperiod=10,
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.ROC(self.df,
                               timeperiod)
...
def add_RSI(self, timeperiod=14,
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.RSI(self.df,
                               timeperiod)
...
def add_TRIX(self, timeperiod=15,
    ...
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.TRIX(self.df,
                                timeperiod)
...
def add_ULTOSC(self, timeperiod=14, timeperiod2=14, timeperiod3=28,
    ...
    name = 'ULTOSC({})'.format(str(timeperiod),
                               ...
                               str(timeperiod3))
    self.sec[name] = dict(type=type, color=color)
    self.ind[name] = talib.ULTOSC(self.df,
                                  timeperiod,
                                  timeperiod2,
                                  timeperiod3)
