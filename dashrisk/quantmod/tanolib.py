...
def WMA(df,n):    
    ...
    df['denom'] = df['Volume'].rolling(n).sum()
    df['numerator'] = df.xy.rolling(n).sum()
    wma = df.apply(lambda r: r.numerator / r.denom,axis=1).as_matrix().reshape(-1)    
    return wma
        

#Exponential Moving Average  
def EMA(df, n):  
    ema = pd.ewma(df['Close'], span = n, min_periods = n - 1).as_matrix().reshape(-1)
    return ema

#Momentum  
def MOM(df, n):  
    return df['Close'].diff(n).as_matrix().reshape(-1)
...
def ATR(df_in, n):  
    ...
    return ATR.as_matrix().reshape(-1)

#Bollinger Bands  
def BBANDS(df, n,nbdevup=2, nbdevdn=2, matype='MA'): 
    ma = pd.rolling_mean(df['Close'], n).as_matrix().reshape(-1)
    msd = pd.rolling_std(df['Close'], n).as_matrix().reshape(-1)
    blower = ma - nbdevdn * msd   
    bhigher = ma + nbdevup * msd   
    return blower,ma,bhigher
...
def TRIX(df_in, n):  
    ...
    return Trix.as_matrix().reshape(-1)

#Average Directional Movement Index  
def ADX(df_in, n, n_adx=None): 
    df = df_in.copy()
    df['timestamp'] = df.index
    df.index = list(range(len(df))) 
    n_ADX = n_adx if n_adx is not None else n
    i = 0  
    UpI = []  
    DoI = []  
    while i + 1 <= df.index[-1]:  
    ...
    return ADX.as_matrix().reshape(-1)

#MACD, MACD Signal and MACD difference  
def MACD(df, n_fast, n_slow,signalperiod=9):  
    EMAfast = pd.Series(pd.ewma(df['Close'], span = n_fast, min_periods = n_slow - 1))  
    EMAslow = pd.Series(pd.ewma(df['Close'], span = n_slow, min_periods = n_slow - 1))  
    MACD = pd.Series(EMAfast - EMAslow, name = 'MACD_' + str(n_fast) + '_' + str(n_slow))  
    MACDsign = pd.Series(pd.ewma(MACD, span = signalperiod, min_periods = signalperiod-1), name = 'MACDsign_' + str(n_fast) + '_' + str(n_slow))  
    MACDdiff = pd.Series(MACD - MACDsign, name = 'MACDdiff_' + str(n_fast) + '_' + str(n_slow))  
    return MACD.as_matrix().reshape(-1),MACDsign.as_matrix().reshape(-1),MACDdiff.as_matrix().reshape(-1)
...
def RSI(df, n):  
    ...
    return RSI.as_matrix().reshape(-1)
...
def MFI(df_in, n):  
    ...
    df = df.join(MFI)  
    return MFI.as_matrix().reshape(-1)
...
def CCI(df, n):  
    ...
    return CCI.as_matrix().reshape(-1)
...
#Ultimate Oscillator  
def ULTOSC(df_in,timeperiod=7,timeperiod1=14,timeperiod2=28):  
    df = df_in.copy()
    df['timestamp'] = df.index
    df.index = list(range(len(df))) 
    ...
    while i < df.index[-1]:  
        ...
        BP_l.append(BP)  
        i = i + 1  
    UltO = pd.Series((4 * pd.rolling_sum(pd.Series(BP_l), timeperiod) / pd.rolling_sum(pd.Series(TR_l), timeperiod)) + (2 * pd.rolling_sum(pd.Series(BP_l), timeperiod1) / pd.rolling_sum(pd.Series(TR_l), timeperiod1)) + (pd.rolling_sum(pd.Series(BP_l), timeperiod2) / pd.rolling_sum(pd.Series(TR_l), timeperiod2)), name = 'Ultimate_Osc')  
    return UltO.as_matrix().reshape(-1)
...
# AROON INDICATORS
def AROON(df,n):
    aup = np.array(aroon_up(df, n))
    adn = np.array(aroon_down(df, n))
    fill = np.repeat([None],len(df)-len(aup))
    aup = np.concatenate((fill,aup))
    adn = np.concatenate((fill,adn))
    return (aup,adn)

def aroon_up(df, period):
    """
    Aroon Up.
    Formula:
    ...
    """
    period = int(period)
    data  = df.Close.as_matrix().reshape(-1)
    a_up = [((period -
